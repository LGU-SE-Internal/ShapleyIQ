...
def run(
    ...
    try:
        ...
        args = AlgorithmArgs(
            input_folder=data_path, traces=traces_lf, metrics=metrics_lf, logs=logs_lf
        )
        ...
        if results:
            ...
            for i, node_name in enumerate(result.ranks):
                ...
                output_data.append(
                    {"rank": i + 1, "node_name": node_name, "score": score}
                )
            ...
            if output:
                ...
                for item in output_data[:10]:  # Show top 10
                    typer.echo(
                        f"{item['rank']:2d}. {item['node_name']:<30} {item['score']:.4f}"
                    )
        else:
