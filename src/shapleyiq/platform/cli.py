...
def run(
    ...
    env_file: Annotated[Optional[Path], typer.Option(help="Environment configuration file")] = None,
    ...
    try:
        ...
        if not all([normal_start, normal_end, abnormal_start, abnormal_end]):
            typer.echo("Warning: Time ranges not found in environment config, using default files")
            ...
            normal_traces = load_traces(traces_file, start_time=normal_start, end_time=normal_end)
            abnormal_traces = load_traces(traces_file, start_time=abnormal_start, end_time=abnormal_end)
        
        if verbose:
            typer.echo(f"Loaded {len(normal_traces)} normal traces and {len(abnormal_traces)} abnormal traces")
        ...
        if metrics_file.exists():
            ...
            try:
                if normal_start and normal_end and abnormal_start and abnormal_end:
                    normal_metrics = load_metrics(metrics_file, start_time=normal_start, end_time=normal_end)
                    abnormal_metrics = load_metrics(metrics_file, start_time=abnormal_start, end_time=abnormal_end)
                else:
                    # Try separate files
                ...
                if verbose and normal_metrics is not None:
                    typer.echo(f"Loaded metrics data")
        ...
        if logs_file.exists():
            ...
            try:
                if normal_start and normal_end and abnormal_start and abnormal_end:
                    normal_logs = load_logs(logs_file, start_time=normal_start, end_time=normal_end)
                    abnormal_logs = load_logs(logs_file, start_time=abnormal_start, end_time=abnormal_end)
                else:
                    # Try separate files
        ...
        if output:
            ...
            for item in output_data[:10]:  # Show top 10
                typer.echo(f"{item['rank']:2d}. {item['node_name']:<30} {item['probability']:.4f}")
...
def validate_data(
    ...
    optional_files = ["metrics.parquet", "logs.parquet", "normal_traces.parquet", "abnormal_traces.parquet"]
